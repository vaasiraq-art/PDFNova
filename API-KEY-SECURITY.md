# Google / Firebase API-key security
...
If a browser-based Firebase or Google API is added later, remember:

- A browser key is not a service-account private key.
- Restrict browser keys by HTTP referrers and, where supported, by the APIs they are allowed to call.
...
Google's current API-key guidance:
